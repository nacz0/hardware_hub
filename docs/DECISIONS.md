...
| Preserve dirty data but guard workflows | Dirty records remain visible for the auditor, but core user actions should not normalize or advance invalid states. Purchase-date sorting treats invalid dates as invalid, and returns require an assigned user. | Some dirty rows remain non-actionable until an admin corrects or reviews them. |
